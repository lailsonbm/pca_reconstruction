...
% Mover para a introdução
% It classifies based on how much a reconstruction of an image is different from the original one, that is, it classifies based on the image reconstruction error. This relies on the fact that PCA aims to minimize the least mean square error for a given set of samples and therefore the reconstruction errors for images similar to the samples are lesser than the reconstruction errors for dissimilar images. So, we train the classifier by doing PCA separately for pedestrians and non-pedestrians. When an image has to be classified, we compare the reconstructions made by both sets of principal components to determine if it contains or not a pedestrian.

...
\title{An analysis of PCA image reconstruction for classification}
...
% TODO Figure with Reconstructions
...
% TODO Figure with three ROC curves. (Legend should cointain AUCs)
% It is surprising to see that the classifier from negative PCs even has a better performance than the classifier from positive PCs.
...
% TODO Figure with distributions from positive and negative samples
...
Again, we plot the reconstruction error distributions, but now segregating error from grayscale PCs and from edges PCs. These plots are shown in Figure XX, that also displays a plot of the combined errors distribution (i.e. grayscale and edge errors summed). At this time, we get a different scenario. All the three plots exhibit a clear boundary that separate samples from different classes. This indicated that there is a lot of redundancy between errors from grayscale PCs and from edge PCs and one of them can be safely removed from the classifier with a minor impact on classification accuracy. Our results indicate that the errors from grayscale PCs generally yields a slightly worse result, as shown in the Table XX. Therefore, it is possible to compose a classifier only with the edge errors and speed up the classification significantly (since two reconstructions will not have to be computed anymore) losing less than $1\%$ of the classification accuracy. Actually, the classifier based only on the reconstruction errors from the edge PCs sometimes performed even better in our experiments (when $k >= 200$), as also shown in Table XX.

% TODO Figure with distributions from grayscale and edges

% TODO Table comparing AUCs
% K GRAY  EDGE  TOTAL
...
% From the description above, we notice that each reconstruction contributes equally to the total error. Therefore, it is assumed that each one has the same relevance to the classification, what, as our experiments show, it is not the case. We found that some reconstructions contribute more to the classification and it is possible to improve the classification accuracy by adjusting their weights when computing the total error.
% but, as our experiments show, this is not the case. It seems 
% To have a better understanding on how this classifier works, we
